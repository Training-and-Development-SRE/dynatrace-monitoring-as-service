...
if [ -z "$1" ]; then export NO_INSTANCES=1
else export NO_INSTANCES=$1 fi
if [ -z "$2" ]; then export BASE_PORT=8081
else export BASE_PORT=$2 fi
if [ -z "$3" ]; then export BUILD_NUMBER=1
else export BUILD_NUMBER=$3 fi
if [ -z "$4" ]; then export TAG_TYPE=0
else export TAG_TYPE=$4 fi
